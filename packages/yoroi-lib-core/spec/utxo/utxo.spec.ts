...
describe('UTxO', () => {
  ...
  describe('from clean state', () => {
    it('should perform sync', async () => {
      ...
      api.getSafeBlock.returns(Promise.resolve(safeBlock.hash))
      ...
      api.getBestBlock.returns(Promise.resolve(bestBlock.hash))

      api.getTipStatusWithReference.withArgs([safeBlock.hash]).returns(
        Promise.resolve({
          result: UtxoApiResult.SUCCESS,
          value: {
            reference: {
      ...
      sinon.default.assert.called(
        utxoStorage.replaceUtxoAtSafePoint.withArgs(utxos, safeBlock.hash)
      )
      sinon.default.assert.called(
        utxoStorage.appendUtxoDiffToBestBlock.withArgs({
          lastBestBlockHash: bestBlock.hash,
          spentUtxoIds: [utxoDiff.diffItems[1].id],
          newUtxos: [(utxoDiff.diffItems[0] as UtxoDiffItemOutput).utxo]
        })
      )
    })
  })
  ...
  describe('state with safe UTxOs and diffs', () => {
    it('should merge into safe state on sync', async () => {
      ...
      utxoStorage.getUtxoDiffToBestBlock.returns(Promise.resolve([localDiff]))
      ...
      sinon.default.assert.called(
        utxoStorage.replaceUtxoAtSafePoint.withArgs(
          newSafeUtxos,
          localDiff.lastBestBlockHash
        )
      )
      sinon.default.assert.called(
        utxoStorage.appendUtxoDiffToBestBlock.withArgs({
  ...
  describe('handle error responses', () => {
    it('should handle when bestblock gets invalidated', async () => {
      ...
      const bestBlockSequence = [
        badBestBlock.hash,
        goodBestBlock.hash
      ].reverse()
      api.getBestBlock.callsFake(() => {
    ...
    it('should handle when safeblock gets invalidated through tip status', async () => {
      ...
      const safeBlockSequence = [
        badSafeBlock.hash,
        goodSafeBlock.hash
      ].reverse()
      api.getSafeBlock.callsFake(() => {
      ...
      api.getTipStatusWithReference.withArgs([badSafeBlock.hash]).returns(
        Promise.resolve({
          result: UtxoApiResult.SAFEBLOCK_ROLLBACK
        })
      )

      api.getTipStatusWithReference.withArgs([goodSafeBlock.hash]).returns(
        Promise.resolve({
          result: UtxoApiResult.SUCCESS,
          value: {
            reference: {
      ...
      // assert
      sinon.default.assert.called(
        utxoStorage.replaceUtxoAtSafePoint.withArgs(utxos, goodSafeBlock.hash)
      )
      sinon.default.assert.called(
        utxoStorage.appendUtxoDiffToBestBlock.withArgs({
          lastBestBlockHash: bestBlock.hash,
  ...
  describe('get available UTxOs', () => {
    it('should get utxos from safe point', async () => {
      ...
      utxoStorage.getUtxoAtSafePoint.returns(
        Promise.resolve({
      ...
      utxoStorage.getUtxoDiffToBestBlock.returns(Promise.resolve([]))
    ...
    it('should get utxos from diff', async () => {
      ...
      utxoStorage.getUtxoAtSafePoint.returns(Promise.resolve(undefined))

      utxoStorage.getUtxoDiffToBestBlock.returns(Promise.resolve(diffs))
